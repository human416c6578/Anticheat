...
public plugin_cfg()
{
	...
	formatex(szLogFile, 63, "%s/hack_%s.log", FilePath, datestr);
...
public FM_CmdStart_Pre(id, uc_handle, seed)
{
	...
	if(fValue > fMaxSpeed && fMaxSpeed>=250.0 )
	{
		bBlockSpeed = true;
	...
	/*if(bBlockSpeed)
	{
		...
		//UTIL_LogUser(id, "Flat!");
		fVelocity[0] *= 0.2;
		fVelocity[1] *= 0.2;
		set_pev(id, pev_velocity, fVelocity);
		
		if(++g_iPunishWarningMove[id] >= MAX_PUNISHWARNING)
		{
			g_iPunishWarningMove[id] = 0;
		}
	}*/
...
public UTIL_LogUser(const id, const szCvar[], any:...)
{
	...
	//server_cmd("kick #%d iesi in mortii tai cu flatu' de aici!", get_user_userid(id));
}
